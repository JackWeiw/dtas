...
def register_cutlass_warp_init_intrin(
    m_dim: int, n_dim: int, dtype: str, cls_name: str, num_warp_m: int, num_warp_n: int
) -> str:
    ...
    def desc(c: T.handle) -> None:
        C = T.match_buffer(c, (m_dim, n_dim), dtype, scope="cutlass.warp.mma")
        with T.block("root"):
            T.reads()
...
def register_gemm_intrin(
    ...
    def desc(a: T.handle, b: T.handle, c: T.handle) -> None:
        ...
        C = T.match_buffer(c, (m_dim, n_dim), out_dtype, scope="cutlass.warp.mma")

        with T.block("root"):
            T.reads(
                C[0:m_dim, 0:n_dim],
                A[0:A_shape_0, 0:A_shape_1],
                B[0:B_shape_0, 0:B_shape_1],
            )
            T.writes(C[0:m_dim, 0:n_dim])
    ...
    def impl(a: T.handle, b: T.handle, c: T.handle) -> None:
        ...
        with T.block("root"):
            ...
            T.evaluate(
                T.cutlass_warp_mma(
                    C.data, "body", read_ptr(A), read_ptr(B), dtype="handle"
                )
            )
            T.evaluate(T.cutlass_warp_mma(C.data, "epilogue", dtype="handle"))
